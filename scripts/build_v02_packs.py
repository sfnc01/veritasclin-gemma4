"""Build the three v0.2 Evidence Packs and write them to examples/.

Run from the project root:
    python scripts/build_v02_packs.py
...
TOPICS = [
    {
        "slug": "malaria_severe_adults_pack",
        "question": (
...
def build_and_save(topic: dict, provider) -> None:
    ...
    print(f"    Cautions: {len(pack.caution_map)}")
...
def main() -> None:
    ...
    print("VeritasClin Field — v0.2 Pack Builder")
    ...
    if settings.gemma_provider == "mock":
        print("\nERROR: GEMMA_PROVIDER=mock. Set GEMMA_PROVIDER=ollama.")
        sys.exit(1)
    ...
    print(f"\n{'='*60}")
    print(f"Done. {len(TOPICS) - len(errors)}/{len(TOPICS)} packs built.")
    if errors:
        for slug, exc in errors:
            print(f"  FAILED: {slug} — {exc}")
        sys.exit(1)
    print("\nNext step: git add examples/ && git commit -m 'feat: add v0.2 demo packs'")
